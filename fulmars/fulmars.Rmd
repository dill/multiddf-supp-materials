...
Now we can make predictions for each of these models. This code is a little complex as it involved binding the predictions onto `pred_sf` which is a spatial object with projection information. We create a `data.frame` that includes flying and swimming abundances combined and individually for the factor and factor-smooth models.
...
# nofactor model
pred_nofactor <- pred
# note that here we need to multiply by 2 to ensure that the results are
# coherent with the other models. We split the segments into the two behaviours
# above, so we need to duplicate here. Note that the proportion is based on
# the segments, (we duplicated the segments so this is easy) and for more
# complex survey schemes this would be a weighted sum
pred_nofactor$Nhat <- 2*predict(vp_nofactor$refit, newdata=pred_nofactor,
