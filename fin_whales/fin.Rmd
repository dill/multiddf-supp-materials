...
Rather than use the analytic approach, it might be preferable to use a Metropolis-Hastings sampler to get a posterior sample for the model. We need to use tools from [`https://github.com/dill/GAMsampling`](https://github.com/dill/GAMsampling) to ensure that the sampling works for the variance-propagated DSM.
